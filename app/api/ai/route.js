...
// Tenta 2.0-flash primeiro; se 429, cai para 1.5-flash (limites diferentes)
...
export async function POST(request) {
  try {
    ...
    for (const model of MODELS) {
      ...
      // 429 neste modelo — tenta o próximo
      if (resp.status === 429) {
      ...
      if (!resp.ok) {
        return NextResponse.json({ error: data.error?.message || `Erro Gemini (${resp.status})` }, { status: resp.status })
      }
