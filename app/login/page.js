...
export default function Login() {
  ...
  async function entrar(e) {
    ...
    if (perfil?.tipo === 'creator') router.push('/creator')
    ...
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg)'}}>
      <div style={{width:400,background:'var(--card)',border:'1px solid var(--rule)',borderRadius:8,overflow:'hidden'}}>
        <div style={{background:'var(--navy)',padding:'28px 32px'}}>
          ...
          <div style={{fontFamily:'var(--serif)',fontSize:'2rem',fontWeight:700,color:'#fff',lineHeight:1.1}}>Creator<br/>Platform</div>
        </div>
        <form onSubmit={entrar} style={{padding:'28px 32px',display:'flex',flexDirection:'column',gap:16}}>
          ...
          <button type="submit" disabled={loading}
            style={{background:'var(--navy)',color:'#fff',border:'none',borderRadius:4,padding:'11px',fontSize:'.82rem',fontWeight:700,marginTop:4,opacity:loading?.6:1}}>
            {loading ? 'Entrando...' : 'Entrar'}
          </button>
          <div style={{textAlign:'center',fontSize:'.74rem',color:'var(--ink3)',paddingTop:8,borderTop:'1px solid var(--rule)'}}>
            Creator nova? <Link href="/cadastro" style={{color:'var(--navy)',fontWeight:600}}>Criar conta</Link>
          </div>
