...
export default function PortalCreator() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      supabase.from('creators').select('*,produtos(nome)').eq('user_id',data.user.id).single().then(({data:c})=>{
        if (!c) return
        setCreator(c)
      ...
      <div style={{padding:'24px 32px'}}>
        ...
        <div style={{display:'flex',gap:0,border:'1px solid var(--rule)',borderRadius:6,overflow:'hidden',marginBottom:20,background:'var(--card)'}}>
          {[['metricas','📊 Métricas'],['roteiros','📝 Roteiros'],['comissoes','💰 Comissões']].map(([id,label])=>(
            <button key={id} onClick={()=>setAba(id)} style={{flex:1,padding:'10px',fontSize:'.76rem',fontWeight:600,color:aba===id?'#fff':'var(--ink3)',background:aba===id?'var(--navy)':'transparent',border:'none',borderRight:'1px solid var(--rule)'}}>
              {label}
            </button>
          ))}
        </div>
            ...
            {campanhas.length===0 ? <div style={{textAlign:'center',padding:40,color:'var(--ink3)',fontSize:'.78rem'}}>Nenhuma campanha ainda.</div> : (
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead><tr style={{borderBottom:'2px solid var(--navy)'}}>
                  {['Período','Leads','Vendas','Conversão','Receita'].map(h=><th key={h} style={{fontSize:'.58rem',textTransform:'uppercase',letterSpacing:'.1em',color:'var(--ink3)',fontWeight:700,padding:'10px 14px',textAlign:'left',background:'var(--bg)'}}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {campanhas.map(c=>{
                    const conv = c.leads>0?(c.pedidos_pagos/c.leads*100).toFixed(1):0
                    return <tr key={c.id} style={{borderBottom:'1px solid var(--rule)'}}>
                      <td style={{padding:'10px 14px',fontWeight:600,fontSize:'.82rem'}}>{c.periodo}</td>
                      ...
                      <td style={{padding:'10px 14px'}}><span style={{fontSize:'.62rem',fontWeight:700,padding:'3px 8px',borderRadius:3,background:parseFloat(conv)>=5?'var(--green-lt)':parseFloat(conv)>=2?'var(--amber-lt)':'var(--red-lt)',color:parseFloat(conv)>=5?'var(--green)':parseFloat(conv)>=2?'var(--amber)':'var(--red)'}}>{conv}%</span></td>
        ...
        {aba==='comissoes' && (
          <div style={{background:'var(--card)',border:'1px solid var(--rule)',borderRadius:6,overflow:'hidden'}}>
            {comissoes.length===0 ? <div style={{textAlign:'center',padding:40,color:'var(--ink3)',fontSize:'.78rem'}}>Nenhuma comissão ainda.</div> : (
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead><tr style={{borderBottom:'2px solid var(--navy)'}}>
                  {['Período','Valor','Status','Data Pgto'].map(h=><th key={h} style={{fontSize:'.58rem',textTransform:'uppercase',letterSpacing:'.1em',color:'var(--ink3)',fontWeight:700,padding:'10px 14px',textAlign:'left',background:'var(--bg)'}}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {comissoes.map(c=>(
                    <tr key={c.id} style={{borderBottom:'1px solid var(--rule)'}}>
                      <td style={{padding:'10px 14px',fontWeight:600,fontSize:'.82rem'}}>{c.mes_referencia}</td>
                      <td style={{padding:'10px 14px',fontSize:'.76rem',color:'var(--green)',fontWeight:600}}>R$ {fmt(c.valor_liquido)}</td>
                      <td style={{padding:'10px 14px'}}><span style={{fontSize:'.6rem',fontWeight:700,padding:'3px 8px',borderRadius:3,background:c.status==='pago'?'var(--green-lt)':'var(--amber-lt)',color:c.status==='pago'?'var(--green)':'var(--amber)'}}>{c.status}</span></td>
                      <td style={{padding:'10px 14px',fontSize:'.76rem',color:'var(--ink3)'}}>{c.data_pagamento||'Pendente'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
